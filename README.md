...
After installing required libraries mentioned above, then you can run the test.py based on provided low-fidelity flow fields generated by FLORIS. We provide three pretrained models which are trained based on 45, 90 and 135 samples (you can choose different models by changing the ```pre_trained_sample``` in test.py). 

You can use the provided low-fidelity flow fields generated by FLORIS under the generation folder (or you can generate new data using gene_floris_farm.py). 
